        ...
        Column {
            ...
            Row(
                ...
                NavigateBarButton(
                    ...
                    color = homeColor,
                    context = context,
                    intent = Intent(context, RecordActivity::class.java)
                )
                NavigateBarButton(
                    bannerMenu = "검색",
...
module gradle Dependencis
```kotlin
implementation ("com.google.android.gms:play-services-maps:18.1.0")
```

compileSdk는 31 이상으로 설정하고 minSdk는 19 이상으로 설정해야 합니다.
```kotlin
android {
    compileSdk 31
...
```
version '2.0.1'에서 오류(따옴표에서 쌍따옴표로 안바꿔서 난 오류 - '2.0.1'도 쌍따옴표로 전환해야 )
...
local.properties(gradle scripts에서 설정로고가 붙어있는 파일 중)
```kotlin
MAPS_API_KEY=YOUR_API_KEY
```

Manifest mergedManifest 클릭-> google.androids.maps 더블 클
```kotlin
...
<application>
        ...
        //밑의 코드 추가 meta-data & uses-permission
        //사용자의 위치에 접근하기 위함
