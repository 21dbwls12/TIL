...
- 멀티플레이 상태에서 다른 캐릭터에게 변화를 주는 액션을 동기화하는 기능
...
- RPC() 함수
  - PhotonView 클래스에 있는 함수
  - 다른 사용자의 화면에 있는 같은 ViewID를 가진 오브젝트와 행동 동기화하기 위해 관련 정보 전달
  - RpcTarget.All
    - 정보를 전달할 대상
    - All: 모든 사용자에게 체력이 변경되었다는 정보 전달
- [PunRPC]
  - 함수 위에 적어져 있어야만 RPC를 통해 정보를 전달받을 수 있음

<br>

### 에러 수정

- 공격한 사용자 화면에서 공격 받은 캐릭터의 피격 애니메이션이 2번 실행됨
  - 총에 맞은 캐릭터 스스로도 함수가 실행, RPC를 통해 실행 총 2번 실행되기 때문
  - 애니메이션을 실행하는 코드가 본인의 캐릭터일 때만 실행되도록 변경
- 공격을 당해 죽은 캐릭터가 총에 맞으면 다시 일어난 애니메이션이 실행됨
  - 총을 쏴도 플레이어로 인식하지 못하도록 죽은 캐릭터의 태그 변경
...
public class PlayerHP : MonoBehaviour
{
    ...
    void RPC_Damaged(float damage)
    {
        ...
        if (pv.IsMine)
        {
            ...
            (*)// 피격당한 캐릭터의 danaged 모션이 2번 실행되는 오류 수정 위해 이동
            (*)// 체력이 남아있다면
            (*)if (hp > 0)
            (*){
                (*)// 피격 애니메이션 실행
        ...
        (*)// 죽은 캐릭터에 총을 쏘면 다시 일어나서 캐릭터가 움직이는 오류 수정 위해 추가
