...
- `Instantiate(GameObject original)` &rarr; original을 복제한 후 생성
- `Instantiate(GameObject original, Transform parent)` &rarr; original을 복제한 후 parent의 자식으로 생성
- `Instantiate(GameObject original, Vector3 position, Quaternion rotation)` &rarr; original을 복제한 후 position 위치에 rotation 각도로 생성
